...
class ApplicationTest {
    ...
    @Test
    fun `access search heroes endpoint, query an empty name, assert empty list as a result`() {
        withTestApplication(moduleFunction = Application::module) {
    ...
    fun `access search heroes endpoint, query non existing hero, assert empty list as a result`() {
        withTestApplication(moduleFunction = Application::module) {
            handleRequest(HttpMethod.Get, "/boruto/heroes/search?name=cumbio").apply {
                assertEquals(
                    expected = HttpStatusCode.OK,
                    actual = response.status()
                )
